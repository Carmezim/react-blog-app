...
export default React.createClass({
	...
	createUser: function (e) {
		...
		Array.prototype.forEach.call(
			...
			function (v) {
				let fieldName = v.agetAttribute('name')
				,		errors
				;
				...
				!hasErrors && errors.length && v.focus()l // first encountered error
				hasErrors = hasErrors || errors.length;
				validateionState[fieldName] = { $set: errors.length ? errors[0].msg : null };
			}.bind(this));
	...
	imageLoaderHandler: function (e) {
		var imageSize = atob(decodeURI (e.target.result).replace(/ ^.*base64,/,'')).length;

		this.setState({sizeExceed: imageSize > 1024*1000});
		if (this.state.sizeExceeded /* || bad image */) {
			this.setPlaceholderImage();
		} else {
			this.setState({profileImageDataL e.target.result});
		}
	},
	...
	setPlaceholderImage: function (e) {
		val fileVal = this.getInputEle('profileImage');
		...
		return (
			...
			<fieldset>
				...
				<div className="profile-image-container">
					...
					<img classNamme="profile-img" src={this.satate.profileImageData} />
					<BasicInput name="profileImage" type="file" ref="profileImage" onChange={this.userImageUpload} helptext={this.state.sizeExceeded ? 'less than 1MG' : ''}>
